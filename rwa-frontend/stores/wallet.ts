...
import { WalletState, UserProfile } from "@/lib/types";
import {
  isConnected,
  requestAccess,
  getAddress,
...
export const useWalletStore = create<WalletStore>((set, get) => ({
  ...
  connect: async () => {
    ...
    try {
      ...
      if (!connectionResult.isConnected) {
        throw new Error(
          "Freighter is not connected. Please enable the extension."
        );
      }
  ...
  switchNetwork: async (network: "testnet" | "mainnet") => {
    ...
    try {
      ...
      const currentNetwork =
        networkResult.network === "PUBLIC" ? "mainnet" : "testnet";
  ...
  startWalletWatcher: () => {
    ...
    walletWatcher.watch((changes) => {
      ...
      const mappedNetwork =
        changes.network === "PUBLIC" ? "mainnet" : "testnet";
  ...
  updateProfile: (profile: UserProfile) => {
    set({ profile });
    localStorage.setItem("pdr_profile", JSON.stringify(profile));
  },
}));
